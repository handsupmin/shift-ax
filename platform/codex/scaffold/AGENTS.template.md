...
- Use `shift-ax run-request` to create the request-scoped topic/worktree, write brainstorming/spec/plan artifacts plus `execution-handoff.json`, and pause at the human planning-review gate. In non-interactive product-shell use, inspect named files and target repo structure first, then pass concrete `--brainstorm-file`, `--spec-file`, and `--plan-file`; do not create placeholder planning artifacts from an empty stdin interview.
- After `shift-ax run-request`, present the localized chat approval packet from `planReviewBrief` or `<topic>/plan-review-brief.md`, including ambiguity decision, acceptance criteria, likely files, verification, risks, and tasks; ask only for `1`, `2`, or `3` using the user's language. Do not end after only printing paths.
...
- In Shift AX Codex sessions, prefer `$onboard`, `$request <text>`, `$export-context`, `$doctor`, `$status`, `$topics`, `$review <topic>`, and `$help` as the visible product-shell commands.
- Native product-shell skill files are installed globally under `~/.codex/skills/<name>/SKILL.md` for: `onboard`, `request`, `export-context`, `doctor`, `status`, `topics`, and `review`. Project-local Shift AX skill copies are legacy cleanup targets.
