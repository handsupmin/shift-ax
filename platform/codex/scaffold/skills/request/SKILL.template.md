...
5. only if they explicitly agree, bootstrap with `shift-ax run-request --request "<request>" --allow-missing-global-context`
6. otherwise bootstrap with `shift-ax run-request --request "<request>"`
...
9. when the user replies `1`, internally record approval with `shift-ax approve-plan`, then immediately resume with `shift-ax run-request --resume --platform codex`; do not show those internal commands unless automation fails and manual recovery is needed
