...
13. when the user replies `1`, internally record approval with `shift-ax approve-plan`, then immediately resume with `shift-ax run-request --resume --platform claude-code`; continue until implementation, verification, review, and commit either complete or hit a concrete blocker; do not show those internal commands unless automation fails and manual recovery is needed
