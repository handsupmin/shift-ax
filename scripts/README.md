...
- `shift-ax run-request --request <text>` bootstraps a topic/worktree, resolves the global index first, runs an interactive planning interview by default, writes brainstorming/spec/plan artifacts plus `execution-handoff.json`, writes `plan-review-brief.md`, and pauses at the human planning-review gate. In non-interactive product-shell use, agents should inspect named request files and repo structure first, then pass concrete `--brainstorm-file`, `--spec-file`, and `--plan-file`; empty non-interactive planning answers are rejected so placeholder artifacts cannot be approved. Product-shell agents should summarize that brief and ask for `1` approve/start, `2` request changes, or `3` reject.
- `shift-ax approve-plan --topic <dir> --reviewer <name> --decision approve|reject` records the human planning-review decision and an approved-plan fingerprint. It is internal UX plumbing for product-shell agents and a recovery/debug command for operators; users should normally approve in chat instead of typing it. If planning identified shared policy/base-context changes, the workflow now pauses in a dedicated policy-sync gate before implementation starts.
...
- `shift-ax run-request --topic <dir> --resume` resumes after approval, reruns review aggregation, auto-generates a Lore-compatible commit message artifact, and auto-commits by default once review passes. It is normally run by the product-shell agent after the user approves in chat; use it directly only for recovery/debugging. Use `--no-auto-commit` to stop at commit-ready for manual inspection. It also accepts `--escalation <kind>:<summary>` / `--clear-escalations` to pause or resume around the three mandatory human-escalation triggers.
