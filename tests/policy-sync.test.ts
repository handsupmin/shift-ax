...
function reviewablePlanWithPolicyUpdate(): string {
  return [
    ...
    '## Acceptance Criteria',
    '',
    '- Auth refresh rotation revokes the previous refresh token safely.',
    '- Shared Auth policy documentation records the new revocation rule before implementation.',
    '',
    ...
    '## Checkpoints',
    '',
    '- Confirm the shared Auth policy update is synced before implementation.',
    ...
    '3. Capture verification evidence before review.',
    '',
    '## Optional Coordination Notes',
    '',
    '- Short implementation slices should use subagent execution.',
    '',
    ...
    await withTempGlobalHome('shift-ax-policy-sync-home-', async () => {
      ...
      const started = await startRequestPipeline({
        ...
        specContent: [
          ...
          '## Constraints',
          '',
          '- Auth policy must be updated before implementation.',
          '- No billing or session UI changes.',
          '',
          '## Out of Scope',
          '',
          '- Billing flows.',
