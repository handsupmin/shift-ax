...
function buildDefaultImplementationPlan(testStrategy: string, architecture: string): string {
  return [
    ...
    '## Acceptance Criteria',
    '',
    '- Replace this with the reviewed acceptance criteria before implementation begins.',
    '',
    ...
    '## Dependencies',
    '',
    '- Review the matched base-context docs and approved plan before editing.',
    '',
    '## Likely Files Touched',
    '',
    '- Replace this with the likely files or paths before implementation begins.',
    '',
    '## Checkpoints',
    '',
    '- Confirm the plan review is still approved and the scope is unchanged before editing.',
    '- Stop the line on unexpected failures, reproduce them first, then add or update a regression test.',
    '- Capture changed files, untouched areas, and tests run before finalization.',
    '',
    '## Execution Tasks',
    '',
    `1. Use ${testStrategy.toUpperCase()} first and keep the first slice focused.`,
    `2. Respect ${architecture.replace(/-/g, ' ')} and keep scope boundaries explicit.`,
    '3. Add focused verification steps before commit finalization.',
    '',
    '## Optional Coordination Notes',
    '',
    '- Short slices should use subagent execution by default.',
    '- Long-running or cross-cutting work should use tmux-backed execution by default.',
    '',
    ...
    '- If a bug or failure is involved, reproduce it first before fixing it.',
    '',
    '## Global Knowledge Updates',
    '',
    '- None yet.',
