...
- **결정적 에이전트 하네스**
  LLM에게 오케스트레이션을 맡기지 않고 FSM, SQLite queue, DAG readiness, retry, idempotency를 스크립트가 소유한 레일 안에서 에이전트 작업을 실행합니다.
...
- `~/.shift-ax/index.md`: 검색 가능한 label, alias, repository, workflow, domain term을 담는 단일 dictionary
- `~/.shift-ax/role/`
- `~/.shift-ax/work-types/`
- `~/.shift-ax/repos/`
- `~/.shift-ax/procedures/`
...
이 단계에서 Shift AX가 당신의 업무 언어와 흐름을 익히기 시작합니다. 온보딩은 관련 레포를 각각 살피고, 머지된 PR 히스토리와 사용자가 준 규칙을 바탕으로 레포별 리뷰 게이트를 만든 뒤, dictionary가 실제 문서로 연결되고 저장된 컨텍스트가 사용 가능한지 검증합니다.
...
Shift AX는 먼저 컨텍스트를 해석하고, 요청 전용 topic/worktree를 만들고, 채팅 안에서 사용자 언어로 된 상세 승인 패킷을 보여줘 `1/2/3` 리뷰를 받은 뒤, 승인되면 구현 / 검증 / 리뷰 / 커밋까지 이어갑니다. 내부 approve/resume 명령은 복구용 도구이지 일반 사용자 경로가 아닙니다.
...
npm에서 `shift-ax@latest`를 다시 설치하고 Codex/Claude 런타임 명령을 전역 위치에 다시 스캐폴딩합니다. 프로젝트별 skill 복사본은 만들지 않습니다.
...
- **계획 준비도**
  각 topic은 ambiguity score를 남기고, 구현 가능한 수준으로 명확하지 않으면 리뷰를 막습니다.

- **독립 리뷰 게이트**
  최종 리뷰는 저장된 아티팩트만 보는 clean-context 게이트를 포함해 커밋 전 온보딩 준수, PRD 증거, 경로 범위, 사이드이펙트 위험, 테스트 관련성을 다시 확인합니다.

- **request-to-commit 루프**
  컨텍스트 해석, 계획, 결정적 오케스트레이션, 구현, 검증, 리뷰, 커밋을 하나의 가이드된 흐름으로 묶는 구조입니다.
