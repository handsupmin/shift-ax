...
- `npm test` 성공
- `npm run build` 성공
- `shift-ax doctor`가 Shift AX repo 기준 `ok`
- target repo가
...
```bash
shift-ax doctor --root /path/to/repo
```
...
자율 실행 LLM이라면, curated onboarding 파일이 이미 없는 한 **`shift-ax --codex` / `shift-ax --claude-code`를 통한 in-shell onboarding**을 우선한다.
