...
- target repo에 아래가 존재
  ...
  - `docs/base-context/` 아래 shared docs
...
- glossary가 켜져 있었다면 `docs/base-context/domain-glossary.md`
- `.ax/project-profile.json`

만약 doctor가 base-context index의 broken path를 보고하면:
...
`docs/base-context/index.md`와 linked docs로 답할 수 있으면 그걸 먼저 쓴다.  
